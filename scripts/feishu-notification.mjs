...
export function createFeishuCard(message, { keyword = '', reportUrl, runUrl, reportLabel = '查看完整报告' } = {}) {
  ...
  const actions = []
  for (const [label, url] of [[reportLabel, reportUrl], ['查看 Actions', runUrl]]) {
    if (url)
  ...
  const template = message.test ? 'blue' : message.summary?.unavailable ? 'orange' : message.summary?.restricted ? 'yellow' : 'green'
  return {
...
export function createFeishuPayload(message, options = {}) {
  const payload = { msg_type: 'interactive', card: createFeishuCard(message, options) }
  if (options.secret) {
...
async function requestFeishu(url, init, fetcher, phase) {
  let response
  try {
