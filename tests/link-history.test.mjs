...
const script = fileURLToPath(new URL('../scripts/restore-link-history.mjs', import.meta.url))
const snapshot = JSON.parse(readFileSync(new URL('../public/status/report.json', import.meta.url), 'utf8'))
...
test('notification is off by default and dry-run never needs SMTP credentials', async () => {
  ...
  const entry = fileURLToPath(new URL('../scripts/notify-link-report.mjs', import.meta.url))
  // Deliberately omit all inherited SMTP configuration.
  const env = { PATH: process.env.PATH, LINK_EMAIL_MODE: 'off' }
  try {
    const disabled = spawnSync(process.execPath, [entry], { cwd: directory, env, encoding: 'utf8' })
    ...
    const preview = spawnSync(process.execPath, [entry, '--dry-run'], { cwd: directory, env: { ...env, LINK_EMAIL_MODE: 'weekly' }, encoding: 'utf8' })
    assert.equal(preview.status, 0)
    assert.match(preview.stdout, /每周友链检测摘要/)
    const missingConfig = spawnSync(process.execPath, [entry], { cwd: directory, env: { ...env, LINK_EMAIL_MODE: 'weekly' }, encoding: 'utf8' })
